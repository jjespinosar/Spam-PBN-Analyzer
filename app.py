...
def main():
    ...
    with st.sidebar:
        ...
        flexibility_level = st.slider(
            ...
            value=6, 
            step=1,
            help="1: Máximo estricto (fácil clasificar como SPAM). 10: Máxima flexibilidad (difícil clasificar como SPAM).",
            key='flexibility_slider'
        )
        
        CONFIG = ajustar_configuracion(flexibility_level)
        
        # Mostrar los ajustes clave de forma clara para el usuario
        factor = (flexibility_level - 1) / 9.0
        st.markdown(f"**Factor de Ajuste (0.0 a 1.0):** `{factor:.2f}`")
        st.markdown(f"**Umbrales de Clasificación Final:**")
        st.markdown(f"**Alto Riesgo:** Puntuación >= **{CONFIG['alto_riesgo']}**")
        st.markdown(f"**Moderado Riesgo:** Puntuación >= **{CONFIG['riesgo_moderado']}**")
        st.markdown(f"*(Los límites internos de los 11 factores se ajustan dinámicamente con el Factor)*")
        ...
        uploaded_file = st.file_uploader(
            ...
            type=['xlsx'], 
            help="El análisis requiere múltiples métricas, por lo que solo se admite el formato Excel estable."
        )
    ...
    if uploaded_file is not None:
        try:
            # 1. Cargar el archivo: FORZAMOS LA LECTURA DE EXCEL
            uploaded_file.seek(0)
            df_original = pd.read_excel(uploaded_file)
            
            # Limpiar y asegurar el DataFrame
            df_original.columns = df_original.columns.astype(str).str.strip() 
            df_original = df_original.reset_index(drop=True)
            
            st.success(f"Archivo **'{uploaded_file.name}'** cargado con {len(df_original)} filas.")
            
            # 2. Preparar datos y ejecutar el análisis
            df_prepared, target_col_name = preparar_datos_spam(df_original.copy())

            if not target_col_name:
                st.error("❌ ERROR: No se pudo identificar la columna de dominio/URL ('Target', 'domain', 'url', etc.). Revise su archivo.")
                return

            df_analysis_results = analizar_spam_vectorizado(df_prepared, CONFIG)
            ...
            # Nombres de columnas finales para la visualización (idéntico al script original)
            columnas_finales = [
                target_col_name,
                'Domain Rating', 
                'Organic / Traffic', 
            ...
            try:
                # Aplicar estilo de gradiente 
                # ESTO REQUIERE MATPLOTLIB
                ...
                # Si falla, muestra la advertencia y la tabla sin estilo, como antes.
        ...
        st.info("⬆️ Por favor, sube tu archivo de dominios en formato Excel (.xlsx) en el panel de la izquierda para comenzar el análisis.")
